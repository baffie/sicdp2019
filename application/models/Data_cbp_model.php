...
class Data_cbp_model extends CI_Model
{

    public $table ='data_cbp';
    public $id = 'id_cbp';
    public $order = 'DESC';
    ...
    /*function grid($id=null)
    {
        ...
        if($id){
            ...
            $this->datatables->add_column('action','<a class="btn btn-sm btn-primary" data-toggle="tooltip" data-placement="top" title="Edit" href="'.site_url().'auth/data_cbp/update/$1"><i class="fa fa-edit"></i></a> <button onclick="return confirmModal(\''.base_url().'auth/data_cbp/delete/$1\')" class="btn btn-sm btn-danger" data-toggle="tooltip" data-placement="top" title="Hapus"><i class="fa fa-trash"></i></button>','id');
        }else{
            $this->datatables->add_column('action','<a class="btn btn-sm btn-primary" data-toggle="tooltip" data-placement="top" title="Edit" href="'.site_url().'cms/data_cbp/update/$1"><i class="fa fa-edit"></i></a> <button onclick="return confirmModal(\''.base_url().'cms/data_cbp/delete/$1\')" class="btn btn-sm btn-danger" data-toggle="tooltip" data-placement="top" title="Hapus"><i class="fa fa-trash"></i></button>','id');
        }
       ...
       function grid($id=null)
       {
        $this->load->database();
        $this->load->library("datatables");
        $this->datatables->select("p.id_cbp as id,
        p.bulan,
        p.tahun, 
        p.stok_awal, 
        p.penambahan, 
        ...
        (p.stok_awal + p.penambahan - p.penyaluran - p.penyusutan) as akhir, ", false)
            ->from($this->table.' p')
            ->join('stok_cbp f', 'f.bulan = p.bulan','left')
        ...
        if($id){
            ...
            $this->datatables->add_column('action','<a class="btn btn-sm btn-info" data-toggle="tooltip" data-placement="top" title="Rekap" href="'.site_url().'auth/data_cbp/rekap/$1"><i class="fa fa-eye"></i></a> <a class="btn btn-sm btn-primary" data-toggle="tooltip" data-placement="top" title="Edit" href="'.site_url().'auth/data_cbp/update/$1"><i class="fa fa-edit"></i></a> <button onclick="return confirmModal(\''.base_url().'auth/data_cbp/delete/$1\')" class="btn btn-sm btn-danger" data-toggle="tooltip" data-placement="top" title="Hapus"><i class="fa fa-trash"></i></button>','id');
     ...
     function grid_filter()
    {
        ...
        $this->datatables->select("p.id_cbp as id,
         ...
         (p.stok_awal + p.penambahan - p.penyaluran - p.penyusutan) as akhir, ", false)
            ->from($this->table.' p');
            //->join('stok_data_cbp f', 'f.bulan = p.bulan','left')
            //->join('bulan b', 'b.bulan = p.bulan', 'left')
            //->join('kabupaten b', 'b.id_kab = p.id_kabupaten', 'left')
            //->join('kecamatan c', 'c.id_kec = p.id_kecamatan', 'left')
            //->join('kelurahan d', 'd.id_kel = p.id_desa', 'left');
...
/* End of file data_cbp_model.php */
/* Location: ./application/models/data_cbp_model.php */