...
class Lpm_model extends CI_Model
{
   ...
   /* function grid($id=null)
    ...
    function grid($id=null)
       {
        ...
        p.penyaluran,
        p.lokasi,  
        (p.stok_awal + p.penambahan - p.penyaluran) as akhir,
        p.created, ", false)
            ->from($this->table.' p')
    ...
    public function get_stok_awal($id_poktan)
    {
        $return = $id_poktan ;
        if($id_poktan)
        {
            ...
            $this->db->order_by('id_lpm','DESC');
            $this->db->limit(1);
            $rs = $this->db->get();
            $data = $rs->result_array();
            $return = $data[0]['stok_awal']+ $data[0]['penambahan'] - $data[0]['penyaluran'];
        } 
        return $return;
    }
