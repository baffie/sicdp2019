...
class Data_cbpkab extends CI_Controller
{
	...
	public function index()
	{
        ...
        $data['source'] = site_url('auth/data_cbpkab/grid');
		...
		$template['page_heading'] = 'Data CBP Kab/kota';
		$template['content'] = $this->load->view('data_cbpkab/list', $data, true);
        $template['js'] = $this->load->view('data_cbpkab/js', $data, true);
    ...
    function grid()
    {
        ...
        echo $this->Data_cbpkab_model->grid($this->user->id);
    }
    ...
    function grid_filter()
    {
        ...
        echo $this->Data_cbpkab_model->grid_filter();
    }

	public function read($id)
	{
		$row = $this->Data_cbpkab_model->get_by_id($id);
		if ($row) {
			$data = array(
				'id_cbpkab' => $row->id_cbpkab,
			...
			$data['load_cities']	= $this->Kabupaten_model->get_category_select(array('' => 'Kabupaten / Kota'));
			$template['page_heading'] = 'Data CBP Kab/kota';
			$template['content'] = $this->load->view('data_cbpkab/read', $data, true);
			$this->load->view('layouts/dashboard', $template);

		} else {
			$this->session->set_flashdata('error', 'Data tidak ditemukan');
			redirect(site_url('auth/data_cbpkab'));
		}
	}
	...
	public function rekap($id=null)
	{
		...
		//print_r ($id);
		//die();
		$poktan = $this->Home_model->get_by_id_cbpkab($id);

		$rdkk = $this->Home_model->get_all_by_cbpkab($id);
		//$kab =substr ($kabupaten, 2);
		$data = array(
			'rdkk_data' => $rdkk,
			'id_cbp' => $poktan->id_cbp,
		...
		$template['page_heading'] = 'DATA CBP KABUPATEN/KOTA';
		$template['content'] = $this->load->view('data_cbpkab/rekap', $data, true);
		$this->load->view('layouts/dashboard', $template);
	}
	 ...
	 public function cetak($id=null)
	{
		...
		$template['content'] = $this->load->view('data_cbpkab/print', $data, true);
	...
	/*public function excel($id=null)
	{
		...
		$template = 'template_excel/data_cbp_kab.xlsx';
		...
		//$data = $this->Home_model->get_all_by_poktan($id);
	   	$id = $this->user->id_kabupaten;
		$poktan = $this->Home_model->get_by_id_cbpkab($id);
		$data = $this->Home_model->get_all_by_cbpkab($id);
	...
	{
		$stok_awal = $this->Data_cbpkab_model->get_stok_cbpkab();
		//die($stok_awal);
		$data = array(
			'button' => 'Tambah',
			'action' => site_url('auth/data_cbpkab/create_action'),
			'id_cbpkab' => set_value('id_cbpkab'),
		...
		$template['page_heading'] = 'Data CBP Kab/kota';
		$template['content'] = $this->load->view('data_cbpkab/form', $data, true);
        $template['js'] = $this->load->view('data_cbpkab/js', $data, true);
		$template['css'] = $this->load->view('data_cbpkab/css', $data, true);
		$this->load->view('layouts/dashboard', $template);
	}
	...
	public function create_action()
	{
		...
		if ($this->form_validation->run() == FALSE) {
			...
			//print_r ($data);
			//die();
			$this->Data_cbpkab_model->insert($data);
	...
	public function update($id)
	{
		...
		if ($row) {
			$data = array(
				'button' => 'Update',
				'action' => site_url('auth/data_cbpkab/update_action'),
				'id_cbpkab' => set_value('id_cbpkab', $row->id_cbpkab),
	...
	public function update_action()
	{
		...
		if ($this->form_validation->run() == FALSE) {
			$this->update($this->input->post('id_cbpkab', TRUE));
		} else {
			$data = array(
				'id_penyuluh' => $this->input->post('id_penyuluh',TRUE),
			...
			$this->Data_cbpkab_model->update($this->input->post('id_cbpkab', TRUE), $data);
	...
	public function delete($id)
	{
		...
		if ($row) {
			$this->Data_cbpkab_model->delete($id);
    ...
    public function valid($id, $status)
    {
        ...
        $this->Data_cbpkab_model->update($id, $data);
        $this->session->set_flashdata('success', 'Data berhasil diupdate');
        redirect('auth/data_cbpkab');
    }
	...
	public function _rules()
	{
		...
		$this->form_validation->set_rules('id_cbpkab', 'id_cbpkab', 'trim');
	...
	public function excel()
	{
		...
		$template = 'template_excel/data_cbp_kab_kota.xlsx';

		$objPHPExcel = PHPExcel_IOFactory::load($template);
		$objPHPExcel->getActiveSheet()->setTitle('Rekapitulasi CBP Kab Kota');
		...
		//$data = $this->Data_cbpkab_model->get_all_by_cbpkab($id_kabupaten);
		$id = $this->user->id_kabupaten;
		$poktan = $this->Home_model->get_by_id_cbpkab($id);
		$data = $this->Home_model->get_all_by_cbpkab($id);
		print_r($data);
		...
		header('Content-Type: application/vnd.ms-excel');
		header('Content-Disposition: attachment;filename="Rekapitulasi_Data_CBP_kab-kota-'.$this->user->name.'-'.$today.'.xlsx"');
		header('Cache-Control: max-age=0');
...
/* End of file data_cbpkab.php */
/* Location: ./application/controllers/data_cbpkab.php */