...
class Ldpm extends CI_Controller
{
	...
	public function index()
	{
		...
		$data['source'] = site_url('auth/ldpm/grid');
		...
		$template['content'] = $this->load->view('ldpm/list', $data, true);
		$template['js'] = $this->load->view('ldpm/js', $data, true);
		$template['css'] = $this->load->view('ldpm/css', $data, true);
	...
	function grid()
	{
		...
		echo $this->Ldpm_model->grid($this->user->id);
	}
	...
	function grid_filter()
	{
		...
		echo $this->Ldpm_model->grid_filter();
	}

	public function read($id)
	{
		$row = $this->Ldpm_model->get_by_id($id);
		if ($row) {
			$data = array(
				'id_ldpm' => $row->id_ldpm,
			...
			$template['page_heading'] = 'Data CPM LDPM';
			$template['content'] = $this->load->view('ldpm/read', $data, true);
			$this->load->view('layouts/dashboard', $template);

		} else {
			$this->session->set_flashdata('error', 'Data tidak ditemukan');
			redirect(site_url('auth/ldpm'));
		}
	}
		...
		public function create()
	{
		$data = array(
			'button' => 'Tambah',
			'action' => site_url('auth/ldpm/create_action'),
			'id_ldpm' => set_value('id_ldpm'),
		...
		$data['load_gpk']	= $this->Gapoktan_model->select_dropdown_gapoktan($data['id_kab']);


		$template['page_heading'] = 'Data CPM LDPM';
		$template['content'] = $this->load->view('ldpm/form', $data, true);
		$template['js'] = $this->load->view('ldpm/js', $data, true);
		$template['css'] = $this->load->view('ldpm/css', $data, true);
		$this->load->view('layouts/dashboard', $template);
	...
	public function update($id)
	{
		...
		if ($row) {
			$data = array(
				'button' => 'Update',
				'action' => site_url('auth/ldpm/update_action'),
				'id_ldpm' => set_value('id_ldpm', $row->id_ldpm),
	...
	public function update_action()
	{
	   ...
	   if($bln=='Januari'){
			...
			$cek = $this->ldpm_model->cek($id_poktan,'Februari',$tahun);
	   }elseif($bln=='Februari'){
			$bln_sblmnya = 'Januari';
			$cek = $this->ldpm_model->cek($id_poktan,'Maret',$tahun);
	   }elseif($bln=='Maret'){
			$bln_sblmnya = 'Februari';
			$cek = $this->ldpm_model->cek($id_poktan,'April',$tahun);
	   }elseif($bln=='April'){
			$bln_sblmnya = 'Maret';
			$cek = $this->ldpm_model->cek($id_poktan,'Mei',$tahun);
	   }elseif($bln=='Mei'){
			$bln_sblmnya = 'April';
			$cek = $this->ldpm_model->cek($id_poktan,'Februari',$tahun);
	   }elseif($bln=='Juni'){
			$bln_sblmnya = 'Mei';
			$cek = $this->ldpm_model->cek($id_poktan,'Juli',$tahun);
	   }elseif($bln=='Juli'){
			$bln_sblmnya = 'Juni';
			$cek = $this->ldpm_model->cek($id_poktan,'Agustus',$tahun);
	   }elseif($bln=='Agustus'){
			$bln_sblmnya = 'Juli';
			$cek = $this->ldpm_model->cek($id_poktan,'September',$tahun);
	   }elseif($bln=='September'){
			$bln_sblmnya = 'Agustus';
			$cek = $this->ldpm_model->cek($id_poktan,'Oktober',$tahun);
	   }elseif($bln=='Oktober'){
			$bln_sblmnya = 'September';
			$cek = $this->ldpm_model->cek($id_poktan,'November',$tahun);
	   }elseif($bln=='November'){
			$bln_sblmnya = 'Oktober';
			$cek = $this->ldpm_model->cek($id_poktan,'Desember',$tahun);
	   }elseif($bln=='Desember'){
			$bln_sblmnya = 'November';
			$cek = $this->ldpm_model->cek($id_poktan,'Januari',$tahun+1);
	   }else{
	   ...
	   if($cek > 0)
	   {
		...
		$stok_awal =  $this->ldpm_model->stok_akhir($id_poktan,$bln_sblmnya,$tahun);*/
		$idkab = $this->user->id_kabupaten;
		$tgl = $this->input->post('tanggal');
		$bln = substr($tgl, 3,-5);
		$thn = substr($tgl, -4,4);
		...
		if ($this->form_validation->run() == FALSE) {
			$this->update($this->input->post('id_ldpm', TRUE));
		} else {
			$data = array(
				'id_penyuluh' => $this->input->post('id_penyuluh',TRUE),
				'id_gapoktan' => $this->input->post('id_gapoktan',TRUE),
			...
			//print_r($data);
			//die();
			$this->Ldpm_model->update($this->input->post('id_ldpm', TRUE), $data);
	...
	public function valid($id, $status)
	{
		...
		$this->Ldpm_model->update($id, $data);
		$this->session->set_flashdata('success', 'Data berhasil diupdate');
		redirect('auth/ldpm');
	}
	...
	public function _rules()
	{
		...
		$this->form_validation->set_rules('id_ldpm', 'id_ldpm', 'trim');
	...
	public function excel()
		{
			...
			$data = $this->Ldpm_model->get_all();

			$objPHPExcel->setActiveSheetIndex(0)->setCellValue('C3', ': ' . $today);
			$objPHPExcel->setActiveSheetIndex(0)->setCellValue('C4', ': ' . $this->user->name);
			$objPHPExcel->setActiveSheetIndex(0)->setCellValue('C5', ': ' . $n, $this->Gapoktan_model->get_by_id($item->id_gapoktan)->nama_gapoktan);

			$i	= 1;
			$n = 7;
			foreach ($data as $item) {
				$objPHPExcel->setActiveSheetIndex(0)->setCellValue('A' . $n, $i);
				$objPHPExcel->setActiveSheetIndex(0)->setCellValue('B' . $n, $this->Gapoktan_model->get_by_id($item->id_gapoktan)->nama_gapoktan);
