...
class Data_cpp_ldpm extends CI_Controller
{
	...
	public function index()
	{
		...
		$data['source'] = site_url('cms/data_cpp_ldpm/grid');
		...
		$template['page_heading'] = 'Data CPP LDPM';
		$template['content'] = $this->load->view('data_cpp_ldpm/list', $data, true);
		$template['js'] = $this->load->view('data_cpp_ldpm/js', $data, true);
		$template['css'] = $this->load->view('data_cpp_ldpm/css', $data, true);
	...
	function grid_filter()
	{
		...
		echo $this->Data_cpp_ldpm_model->grid_filter();
	}

	public function read($id)
	{
		$row = $this->Data_cpp_ldpm_model->get_by_id($id);
		if ($row) {
			$data = array(
				'id' => $row->id,
			...
			$template['page_heading'] = 'Data CPP LDPM';
			$template['content'] = $this->load->view('data_cpp_ldpm/read', $data, true);
			$this->load->view('backend/layouts/dashboard', $template);

		} else {
			$this->session->set_flashdata('error', 'Data tidak ditemukan');
			redirect(site_url('cms/data_cpp_ldpm'));
		}
	}
	...
	public function create()
	{
		...
		$template['page_heading'] = 'Data CPP LDPM';
		$template['content'] = $this->load->view('data_cpp_ldpm/form', $data, true);
		$template['js'] = $this->load->view('data_cpp_ldpm/js', $data, true);
		$template['css'] = $this->load->view('data_cpp_ldpm/css', $data, true);
		$this->load->view('backend/layouts/dashboard', $template);
	}
	...
	public function create_action()
	{
		...
		if ($this->form_validation->run() == FALSE) {
			...
			$this->Data_cpp_ldpm_model->insert($data);
	...
	public function update($id)
	{
		...
		if ($row) {
			$data = array(
				'button' => 'Update',
				'action' => site_url('cms/data_cpp_ldpm/update_action'),
				'id' => set_value('id_ldpm', $row->id),
				'id_penyuluh' => set_value('id_penyuluh', $row->id_penyuluh),
				'id_gapoktan' => set_value('id_gapoktan', $row->id_gapoktan),
				'id_kabup' => set_value('id_kab', $row->id_kab),
	...
	public function update_action()
	{
	   ...
	   if($bln=='Januari'){
			...
			$cek = $this->data_cpp_ldpm_model->cek($id_poktan,'Februari',$tahun);
	   }elseif($bln=='Februari'){
			$bln_sblmnya = 'Januari';
			$cek = $this->data_cpp_ldpm_model->cek($id_poktan,'Maret',$tahun);
	   }elseif($bln=='Maret'){
			$bln_sblmnya = 'Februari';
			$cek = $this->data_cpp_ldpm_model->cek($id_poktan,'April',$tahun);
	   }elseif($bln=='April'){
			$bln_sblmnya = 'Maret';
			$cek = $this->data_cpp_ldpm_model->cek($id_poktan,'Mei',$tahun);
	   }elseif($bln=='Mei'){
			$bln_sblmnya = 'April';
			$cek = $this->data_cpp_ldpm_model->cek($id_poktan,'Februari',$tahun);
	   }elseif($bln=='Juni'){
			$bln_sblmnya = 'Mei';
			$cek = $this->data_cpp_ldpm_model->cek($id_poktan,'Juli',$tahun);
	   }elseif($bln=='Juli'){
			$bln_sblmnya = 'Juni';
			$cek = $this->data_cpp_ldpm_model->cek($id_poktan,'Agustus',$tahun);
	   }elseif($bln=='Agustus'){
			$bln_sblmnya = 'Juli';
			$cek = $this->data_cpp_ldpm_model->cek($id_poktan,'September',$tahun);
	   }elseif($bln=='September'){
			$bln_sblmnya = 'Agustus';
			$cek = $this->data_cpp_ldpm_model->cek($id_poktan,'Oktober',$tahun);
	   }elseif($bln=='Oktober'){
			$bln_sblmnya = 'September';
			$cek = $this->data_cpp_ldpm_model->cek($id_poktan,'November',$tahun);
	   }elseif($bln=='November'){
			$bln_sblmnya = 'Oktober';
			$cek = $this->data_cpp_ldpm_model->cek($id_poktan,'Desember',$tahun);
	   }elseif($bln=='Desember'){
			$bln_sblmnya = 'November';
			$cek = $this->data_cpp_ldpm_model->cek($id_poktan,'Januari',$tahun+1);
	   }else{
	   ...
	   if($cek > 0)
	   {
		...
		$stok_awal =  $this->data_cpp_ldpm_model->stok_akhir($id_poktan,$bln_sblmnya,$tahun);*/
		$tgl = $this->input->post('tanggal');
		$bln = substr($tgl, 3,-5);
		$thn = substr($tgl, -4,4);
		...
		if ($this->form_validation->run() == FALSE) {
			...
			if (!empty($_FILES['attach_file']) && !empty($_FILES['attach_file']['name'])) {
				...
				if (!$this->upload->do_upload('attach_file')) {
					$this->session->set_flashdata('error', $this->upload->display_errors());
					redirect(site_url('cms/data_cpp_ldpm/update'));
				} else {
					$this->load->library('image_lib');
					$upload_data = $this->upload->data();
			...
			//print_r($data);
			$this->Data_cpp_ldpm_model->update($this->input->post('id', TRUE), $data);
	...
	public function delete($id)
	{
		...
		if ($row) {
			$this->Data_cpp_ldpm_model->delete($id);
	...
	public function valid($id, $status)
	{
		...
		$this->Data_cpp_ldpm_model->update($id, $data);
		$this->session->set_flashdata('success', 'Data berhasil diupdate');
		redirect('cms/data_cpp_ldpm');
	}
	...
	public function _rules()
	{
		...
		//$this->form_validation->set_rules('attach_file', 'attach_file', 'trim|required');
	...
	public function excel()
		{
			...
			$template = 'template_excel/cpp_ldpm.xlsx';
			...
			$data = $this->Data_cpp_ldpm_model->get_all();
			...
			foreach ($data as $item) {
				...
				$objPHPExcel->setActiveSheetIndex(0)->setCellValue('C' . $n, $this->Kabupaten_model->get_by_id($item->id_kab)->nama_kab);
				$objPHPExcel->setActiveSheetIndex(0)->setCellValue('D' . $n, $item->bulan);
				$objPHPExcel->setActiveSheetIndex(0)->setCellValue('E' . $n, $item->tahun);
				$objPHPExcel->setActiveSheetIndex(0)->setCellValue('F' . $n, $item->stok_awal);
				$objPHPExcel->setActiveSheetIndex(0)->setCellValue('G' . $n, $item->penambahan);
				$objPHPExcel->setActiveSheetIndex(0)->setCellValue('H' . $n, $item->penyaluran);
				$objPHPExcel->setActiveSheetIndex(0)->setCellValue('I' . $n, $item->penyusutan);
				$objPHPExcel->setActiveSheetIndex(0)->setCellValue('J' . $n, $item->stok_awal + $item->penambahan - $item->penyaluran - $item->penyusutan);
				++$n;
				++$i;
			}
			...
			$objPHPExcel->getActiveSheet()->getStyle('A7:J' . $highestRow)->applyFromArray(
				array(
					'font' => array(
						'bold' => false,
						'size'  => 11,
			...
			header('Content-Disposition: attachment;filename="Rekapitulasi_CPP_LDPM-'.$this->user->name.'-'.$today.'.xlsx"');
...
/* End of file data_cpp_ldpm.php */
/* Location: ./application/controllers/data_cpp_ldpm.php */