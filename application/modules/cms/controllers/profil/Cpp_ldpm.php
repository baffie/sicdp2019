...
class Cpp_ldpm extends MX_Controller
{
    ...
    public function index()
    {
        ...
        $poktan = $this->Home_model->get_search_cpp_ldpm($kabupaten);
        //$poktan = $this->Home_model->get_by_cpp_ldpm($poktan->id_gapoktan);
        $data = array(
            'data_poktan' => $poktan
        );
        ...
        $template['page_heading'] = 'Profil CPP LDPM';
        $template['content'] = $this->load->view('profile/v_cpp_ldpm', $data, true);
    ...
    public function view($slug=null)
    {
        ...
        $poktan = $this->Home_model->get_by_slug_cpp_ldpm($slug);
        $get_stok_awal = $this->Home_model->get_stok_awal_cpp_ldpm($poktan->id_gapoktan);
        $rdkk = $this->Home_model->get_all_by_cpp_ldpm($poktan->id_gapoktan);
        ...
        $template['content'] = $this->load->view('v_cpp_ldpm_detail', $data, true);
    ...
    public function data($slug=null)
    {
        ...
        $template['title'] = 'PROFIL CPM LPM';
        $template['content'] = $this->load->view('v_cpp_ldpm_profil', $data, true);
        $template['js'] = $this->load->view('js', $data, true);
        $this->load->view('frontend/layouts/print', $template);
    }
         ...
         public function set()
    {
        ...
        redirect(site_url('profil/cpp_ldpm'));
    ...
    public function excel($id=null)
    {
        ...
        $objPHPExcel->getActiveSheet()->setTitle('Rekapitulasi CPP LDPM Gapoktan');

        $poktan = $this->Home_model->get_by_id_cpp_ldpm($id);
        $data = $this->Home_model->get_all_by_cpp_ldpm($id);
   ...
   $objPHPExcel->setActiveSheetIndex(0)->setCellValue('A' . $n, $i);
        ...
        header('Content-Disposition: attachment;filename="CPP_Gabungan_Kelompok_Tani-'.$poktan->nama_gapoktan.'-'.$today.'.xlsx"');
