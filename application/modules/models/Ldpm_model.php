...
class Ldpm_model extends CI_Model
{
    ...
    function get_all($id=null)
    {
       ...
       function grid($id=null)
       {
        ...
        p.penyaluran, 
        p.penyusutan,
        (f.stok_awal + p.penambahan - p.penyaluran - p.penyusutan) as akhir, ", false) 
            ->from($this->table.' p')
        ...
        if($id){
            ...
            $this->datatables->add_column('action','<a class="btn btn-sm btn-info" data-toggle="tooltip" data-placement="top" title="Rekap" href="'.site_url().'auth/ldpm/rekap/$1"><i class="fa fa-eye"></i></a> <a class="btn btn-sm btn-primary" data-toggle="tooltip" data-placement="top" title="Edit" href="'.site_url().'auth/ldpm/update/$1"><i class="fa fa-edit"></i></a> <button onclick="return confirmModal(\''.base_url().'auth/ldpm/delete/$1\')" class="btn btn-sm btn-danger" data-toggle="tooltip" data-placement="top" title="Hapus"><i class="fa fa-trash"></i></button>','id');
        }else{
